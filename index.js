...
// if (-1 || 0) alert( 'first' ); //  выполнится, т.к. true или false. -1 это не falsy значение
// if (-1 && 0) alert( 'second' ); // Не выполнится, т.к. true И false
// if (null || -1 && 1) alert( 'third' ); //  выполнится, т.к. null и -1, а -1 это не falsy 
...
// Иначе – «Неверный пароль», При отмене или в случае если ничего не введено – «Отменено».

// let userLogin = prompt('Введите логин', '');
// if (userLogin == 'Админ'){
//     let password = prompt('Введите ароль','');
...
// А ?? В будет равно А если А определено, будет равно В если А не определено.
// Определено это значит имеет значение, не равно null и undefined. 

// Оператор нулевого слияния возвращает первый аргумент если он не null\undefinde, иначе возвращает второй.
// Позволяет удобно получать значения если они определены

// ruslt = a ?? b; // полная запись нулевого слияния
// result = (a == null || a == undefined) ? a, b; // Аналог нулевого слияния, выполненный обычными функциями

// Где это может быть полезно 

// Обычно ?? используется что бы задать значнеие по умолчанию для потенциально неопределённых переменных.

// let user; // создаём переменную, но не присваиваем ей значения
// alert (user ?? 'Аноним'); // выдаст аноним, т.к. user udefined.

// let user = "Иван" ;
// alert (user ?? "Аноним"); // Иван, т.к. это значение user, эта переменная определена.

// Можно записать последовательность операторов ?? что бы получать первое значение из списка, которое не является null.
 
// let firstName = null;
// let lastName;
// let nickName = "Говнокодер";
// alert (firstName ?? lastName ?? nickName ?? 'Анонимная собака'); // выдаст говнокодер, т.к. это первое значение в цепочке которое определено.

// Для этой задачи || подойдёт так же, ответ будет тот же

// let firstName = null;
// let lastName;
// let nickName = "Говнокодер";
// alert (firstName || lastName || nickName || 'Анонимная собака'); // выдаст говнокодер, т.к. это первое значение в цепочке которое true

// Разница между || и ?? будет втом, что || вернёт первое true, а ?? первое не null и не undefined .
// || не различает между собой false, 0, "", undefined, null.  Если первым будет любой из этих аргументов, то || всегда выберет второй.
// Но на практике часто требуется давать значение по умолчанию только тогда, когда переменная равна null \ undefined. 
// Для этого уже удобно использовать ??

// let height = 0;
// alert (height || 100); // 100, т.к. 0 это falsy, а || выберет лишь true.
// alert (height ?? 100); // 0, т.к. 0 это не null или undefined, а определенный number. 

// У || и ?? низкий приоритет. Используй скобки.

// По соображениям безопасности JS не даёт использовать ?? с || или && если приоритет не указан при помощи круглых скобок.
// Например: let x = 1 ?? 2 && 3; - выдаст синтаксическую ошибку. 
// Сделано это было только ради того что бы люди при прееходе с || на ?? не плодили программные ошибки.

// let x = (1 && 2) ?? 3; // уже работает без ошибок;
// alert (x) // ответом будет 2. 
